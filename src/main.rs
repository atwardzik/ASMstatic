...
const KEYWORDS_WITH_ARGS: [&str; 70] = [
...
fn main() {
    ...
    for line in contents.split('\n') {
        if is_instruction_format(line) {
            println!("{}", normalize_keyword_spacing(line));
        } else {
            println!("{}", line);
        }
    }
...
fn is_instruction_format(line: &str) -> bool {
...
fn normalize_keyword_spacing(line: &str) -> String {
